...
#ifndef CGAL_STD_HASH_MAP
#define CGAL_STD_HASH_MAP
...
public:
    ...
    Std_hash_map(const mapped_type& def)
        : _default(def)
    {
    }

    Std_hash_map(const mapped_type& def,size_type table_size)
        : _default(def)
    {
        Base::reserve(table_size);
    }

    Std_hash_map(key_type key_begin,key_type key_end,mapped_type values_begin,
